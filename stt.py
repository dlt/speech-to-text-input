...
def main():
    ...
    print(f"\n🔧 Active Models:")
    print(f"   • Vosk EN: {en_model_info['name']} ({args.model_en})")
    print(f"   • Vosk PT: {pt_model_info['name']} ({args.model_pt})")
    print(f"   • Whisper: {args.whisper_model}")
        ...
        while True:
            ...
            if rec_en.AcceptWaveform(amplified_data):
                ...
                if text:  # Only print if there's text
                    if DEBUG:
                        print(f"[EN MODEL - {en_model_info['name']}] → {text}")
                    if "transcribe" in text:
                        print(f"🟢 Wake word detected: 'transcribe' (using {en_model_info['name']})")
                        return "en"
            else:
                # Get partial results
                if DEBUG:
            ...
            if rec_pt.AcceptWaveform(amplified_data):
                ...
                if text:  # Only print if there's text
                    if DEBUG:
                        print(f"[PT MODEL - {pt_model_info['name']}] → {text}")
                    if "transcreva" in text:
                        print(f"🟢 Wake word detected: 'transcreva' (using {pt_model_info['name']})")
                        return "pt"
            else:
                # Get partial results
                if DEBUG:
    ...
    try:
        while True:
            ...
            if DEBUG:
                print(f"🧠 Transcribing with Whisper ({args.whisper_model} model) in {lang.upper()}...")
            else:
                print(f"🧠 Using Whisper {args.whisper_model} model for {lang.upper()} transcription...")
            ...
            # Reset recognizers for next wake word detection
            rec_en.Reset()
            rec_pt.Reset()
